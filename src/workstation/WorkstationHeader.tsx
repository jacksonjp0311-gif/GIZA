import {teach} from '../tutorial/Tutorial';
...
export function WorkstationHeader({model,surface,onSurface,modelTitle='KHAFRE / PYRAMID CORE',sphinx=false}: {
  ...
  return <header className="topbar edgeTop compactModelHeader">
    ...
    <nav data-tutorial-id="surface-switch" className="surfaceSwitch" aria-label="Viewport surface">
      <button data-tutorial-id="surface-model" aria-pressed={surface==='MODEL'} className={surface==='MODEL'?'active':''} onClick={()=>onSurface('MODEL')}>3D MODEL</button>
      <button data-tutorial-id="surface-atlas" aria-pressed={surface==='ATLAS'} className={surface==='ATLAS'?'active':''} onClick={()=>onSurface('ATLAS')}>MAP ATLAS</button>
      <button data-tutorial-id="surface-registration" aria-pressed={surface==='REGISTRATION'} className={surface==='REGISTRATION'?'active':''} onClick={()=>onSurface('REGISTRATION')}>REGISTRATION</button>
    <button onClick={()=>teach(0)}>Tutorial / ?</button></nav>
