...
export function ComponentWorkbench({model,part,initialContext,onClose,onOpen,onSelect}:{
  ...
  return <div className="componentWorkbench">
    ...
    <div className="componentStage">
      ...
      <div className="componentControls">
        {showLid&&<label>Lid lift <input aria-label="Sarcophagus lid lift" type="range" min="0" max="1.5" step=".01" value={lidLift} onChange={e=>setLidLift(Number(e.target.value))}/><span>{lidLift===0?'Hidden':(lidLift*1.3).toFixed(2)+' m'}</span></label>}
