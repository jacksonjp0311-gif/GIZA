...
function Region({id,explosion,onSelect,children}:{id:RegionId;explosion:number;onSelect:SphinxSceneProps['onSelect'];children:ReactNode}){
  ...
  return <group ref={ref} name={id} userData={{region:id,geometry_authority:'ILLUSTRATIVE_RECONSTRUCTION'}} onClick={e=>{if(e.delta<4){e.stopPropagation();onSelect(id);}}}>{children}</group>;
}
function Repairs({explosion,clip,wireframe,onSelect,selectedBlock}:{explosion:number;clip:THREE.Plane[];wireframe:boolean;onSelect:SphinxSceneProps['onSelect'];selectedBlock?:number}){
  ...
  return <instancedMesh ref={ref} args={[undefined,undefined,blocks.length]} onClick={(e:ThreeEvent<MouseEvent>)=>{if(e.delta<4){e.stopPropagation();onSelect('repairs',e.instanceId);}}}>
    <boxGeometry/><meshStandardMaterial color="#dec493" roughness={.93} clippingPlanes={clip} wireframe={wireframe}/>
  </instancedMesh>;
}
...
function Geometry(p:SphinxSceneProps){
  ...
  const visible=(id:RegionId)=>!p.hidden.includes(id)&&(!p.isolated||p.selected===id);
  ...
  return <>
    ...
    {p.dimensions&&p.selected==='stela'&&visible('stela')&&<group position={regionOffset('stela',p.explosion)}><Line points={[[23.5,-1.6,0],[23.5,-1.6,3.5]]} color="#f3cd79"/><Html position={[23.5,0,4.1]} center><span className="detailDimension">≈ 3.5 m · published height</span></Html></group>}
    {p.dimensions&&!p.isolated&&p.selected!=='stela'&&<><Line points={[[-33,-16,.1],[40.5,-16,.1]]} color="#f3cd79"/><Html position={[4,-16,1]} center><span className="detailDimension">≈ 73.5 m · published overall length</span></Html><Line points={[[23,13,0],[23,13,20]]} color="#f3cd79"/><Html position={[23,13,21]} center><span className="detailDimension">≈ 20 m · height</span></Html></>}
  </>;
}
