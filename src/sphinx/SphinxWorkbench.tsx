...
export function SphinxWorkbench({onClose,initialArtifact=false}:{onClose:()=>void;initialArtifact?:boolean}){
  ...
  const choose=(id:RegionId,index?:number)=>{setSelected(id);setBlock(index);if(isolated)setRevision(v=>v+1);};
  const fit=()=>setRevision(v=>v+1);
  const inspectArtifact=()=>{setSelected('stela');setBlock(undefined);setHidden(v=>v.filter(id=>id!=='stela'));setIsolated(true);setFocused('stela');setExplosion(0);setSection(null);setGeology(false);setView('Front');setDimensions(true);setPhotos(true);setTurntable(false);fit();};
  const reset=()=>{setExportStatus('');setExplosion(0);setSelected(null);setBlock(undefined);setHidden(['repairs']);setIsolated(false);setGeology(false);setWireframe(false);setSection(null);setDimensions(false);setGround(true);setTurntable(false);setView('Perspective');setFocused(null);fit();};
  ...
  const exportView=()=>{
    const payload={schema:'giza.sphinx.study-view.v1',geometry_authority:'ILLUSTRATIVE_RECONSTRUCTION',not_a_survey:true,exported_at:new Date().toISOString(),view,focused,explosion,selected,block,isolated,hidden,section,geology,wireframe,dimensions,sources:SPHINX_SOURCES};
  ...
  const exportMesh=async()=>{
    ...
    try{
      ...
      snapshot.userData={...snapshot.userData,frame:'glTF Y-up: X east, Y up, -Z north; no global registration',sources:SPHINX_SOURCES.map(s=>s.url)};
  ...
  return <section className={`sphinxWorkbench${focus?' sphinxFocus':''}`} data-sphinx-workbench>
    <aside className="sphinxPanel">
      ...
      <SpecificationsPanel/>
      <button onClick={()=>{setHidden(['repairs','stela']);setSelected('core');setIsolated(false);setFocused(null);setGeology(true);setExplosion(0);setSection(null);fit();}}>Expose carved bedrock</button>
    ...
    <div className="sphinxViewer">
      ...
      <div className={`sphinxStage${photos?' withReferences':''}`}><div className="sphinxCanvas"><SphinxScene explosion={explosion} selected={selected} selectedBlock={block} isolated={isolated} focused={focused} hidden={hidden} geology={geology} wireframe={wireframe} section={section} ground={ground} dimensions={dimensions} turntable={turntable} view={view} revision={revision} onSelect={choose} onReady={s=>{scene.current=s;}}/>
        <span className="sphinxBadge">EXTERIOR RECONSTRUCTION · NOT A SCAN</span>
      ...
      </div>{photos&&<SphinxReferences region={selected} onClose={()=>setPhotos(false)} onInspect={(id,camera)=>{setSelected(id);setBlock(undefined);setFocused(id==='core'?null:id);setIsolated(false);setHidden(v=>v.filter(h=>h!==id));setExplosion(0);setSection(null);setView(camera);fit();}}/>}</div>
      <div className="sphinxTools">
        ...
        <label><input type="checkbox" checked={!hidden.includes('repairs')} onChange={e=>setHidden(v=>e.target.checked?v.filter(id=>id!=='repairs'):[...v.filter(id=>id!=='repairs'),'repairs'])}/>Repair-block study</label>
