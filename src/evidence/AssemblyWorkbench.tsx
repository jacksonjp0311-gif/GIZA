...
export default function AssemblyWorkbench({model,initialPart,onClose:onCloseRequested,onLegacy:onLegacyRequested}:{model:ModelBundle;initialPart:string;onClose:()=>void;onLegacy:()=>void}){
  ...
  const onCameraReader=useCallback((read:()=>CameraBookmark|null)=>{cameraReader.current=read;},[]);
  ...
  const pick=(f:EvidenceFeature,p:CanonicalPoint)=>{
    ...
    const anchors=featureAnchors(f),chosen=snap&&!p.origin&&anchors.length?anchors.reduce((a,b)=>Math.hypot(...a.map((n,i)=>n-p.position[i]))<Math.hypot(...b.map((n,i)=>n-p.position[i]))?a:b):p.position;
    const point={...p,position:chosen};setPoints(v=>v.length>=(pickMode==='ANGLE'?3:2)?[point]:[...v,point]);
  };
  // The DOM tool panel and the R3F root commit independently. Route real picks through
  // the latest committed research mode, never a stale renderer callback.
  const pickCurrent=useRef(pick);useLayoutEffect(()=>{pickCurrent.current=pick;});
  const dispatchPick=useCallback((f:EvidenceFeature,p:CanonicalPoint)=>pickCurrent.current(f,p),[]);
