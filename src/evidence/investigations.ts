...
export interface SavedInvestigation {schema:'giza.saved-investigation.v1'|'giza.saved-investigation.v2';id:string;sha256:string;payload:{title:string;createdAt:string;assemblySnapshot:EvidenceAssembly;snapshotSha256:string;dependencyFingerprint:string;dependencyRule?:'canonical-measurement.v1'|'canonical-measurement.v2';draft:InvestigationDraft;result:SpatialResult;receipts:EvidenceReceipt[];presentation:InvestigationPresentation}}
function checkDraft(d:InvestigationDraft,a:EvidenceAssembly){
  ...
  for(const p of d.points){
    ...
    if(g.kind==='box'&&(p.position.some((v,i)=>v<g.min[i]-1e-5||v>g.max[i]+1e-5)||(!p.origin&&!p.position.some((v,i)=>Math.abs(v-g.min[i])<1e-5||Math.abs(v-g.max[i])<1e-5))))throw new Error('Point is outside its originating physical surface');
    if(p.origin){
...
async function fingerprint(a:EvidenceAssembly,d:InvestigationDraft,rule='canonical-measurement.v1'){
  if(!['canonical-measurement.v1','canonical-measurement.v2'].includes(rule))throw new Error('Unsupported measurement dependency rule');
  ...
  return sha256Json({rule,points:d.points,mode:d.mode,frameId:d.frameId,features:rule==='canonical-measurement.v2'?features.map(({label:_,...physical})=>physical):features,frames:rule==='canonical-measurement.v2'?frameRecords.map(({label:_,...physical})=>physical):frameRecords,transforms:ts,observations:a.observations.filter(o=>observations.has(o.id))});
}
export async function saveInvestigation(title:string,assembly:EvidenceAssembly,draft:InvestigationDraft,presentation:InvestigationPresentation,receipts:readonly EvidenceReceipt[]):Promise<SavedInvestigation>{
  ...
  const payload={title,createdAt:new Date().toISOString(),assemblySnapshot:a,snapshotSha256:await sha256Json(a),dependencyRule:'canonical-measurement.v2',dependencyFingerprint:await fingerprint(a,draft,'canonical-measurement.v2'),draft:physicalDraft,result:compute(a,draft),receipts:verified,presentation:{...presentation,...(presentation.selectedId||selectedId?{selectedId:presentation.selectedId??selectedId}:{})}};
...
export async function investigationApplicability(s:SavedInvestigation,current:EvidenceAssembly){
  try{const checked=await restoreInvestigation(s);checkDraft(checked.payload.draft,current);return await fingerprint(current,checked.payload.draft,checked.payload.dependencyRule)===checked.payload.dependencyFingerprint?'CURRENT':'HISTORICAL';}catch{return 'UNVERIFIABLE';}
}
