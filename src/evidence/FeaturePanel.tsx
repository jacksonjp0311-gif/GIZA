...
export function FeaturePanel({assembly,graph,feature,onSelect,layers,model}:{assembly:EvidenceAssembly;graph:SpatialEvidenceGraph;feature:EvidenceFeature;onSelect:(id:string)=>void;layers:Record<RealityAuthority,boolean>;model:ModelBundle}){
  ...
  return <>
    ...
    {graph.nodes.filter(n=>n.kind==="PLAN_RELATION"&&Array.isArray(n.data.affectedFeatures)&&n.data.affectedFeatures.includes(feature.id)).map(n=><p key={n.id} className="evidenceWarning">Linked PLAN_2D context: {n.label}. This is not a 3-D surface registration. Open Compare for local replay/review state; imported claims require replay.</p>)}
