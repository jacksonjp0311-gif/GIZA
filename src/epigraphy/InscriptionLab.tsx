...
export default function InscriptionLab({onClose,onModel}:{onClose:()=>void;onModel?:()=>void}){
  ...
  const [pendingExit,setPendingExit]=useState<{action:()=>void;label:string}|null>(null);
  ...
  const requestExit=(action:()=>void,label:string)=>{if(dirty.current)setPendingExit({action,label});else action();};
  ...
  return <dialog ref={dialog} className="epigraphyLab" aria-labelledby="epigraphy-title" data-epigraphy onClose={onClose} onCancel={event=>{event.preventDefault();requestExit(onClose,'Leave without saving');}}>
    ...
    {pendingExit&&<section className="epiSaveWarning" role="alert"><b>Unsaved notebook changes</b><p>Export your draft before leaving or replacing it. An export starts a download; confirm you have kept the file.</p><button onClick={exportDraft}>Export current draft</button><button onClick={()=>setPendingExit(null)}>Keep editing</button><button onClick={()=>{const action=pendingExit.action;setPendingExit(null);action();}}>{pendingExit.label}</button></section>}
