...
if(fs.existsSync('EVIDENCE_ASSEMBLY_MANIFEST.json')){
 ...
 const paths=new Set([...(d.artifacts??[]).map(r=>r.path),...tree('src/evidence'),...tree('src/tutorial'),'src/workstation/navigationGuard.ts','tests/live-evidence.test.mjs','docs/TUTORIAL_MODE.md','docs/LIVE_EVIDENCE_INTEGRATION.md',...tree('scripts/evidence'),...tree('tests/browser'),'tests/evidence-boundaries.test.mjs','tests/evidence-workflow.test.mjs','tests/evidence-campaign.test.mjs','docs/EVIDENCE_BOUNDARY_EVOLUTION.md','docs/SOURCE_CAMPAIGN.md','docs/RELIABILITY_0_11_1.md','public/model/evidence_assembly/campaign-khafre-plan-blocked-20260917.json']);
 d.version=pkg.version;d.schemas={assembly:'giza.evidence-assembly.v1',quantity:'giza.quantity.v1',receipt:'giza.evidence-receipt.v2',investigation:'giza.saved-investigation.v2',campaign:'giza.scoped-campaign.v1',comparisonRule:'giza.comparison-rules.v3',measurementRule:'canonical-measurement.v4',transformRule:'giza.transform-authority.v2',conditionalMeasurement:'giza.conditional-measurement.v1',livePlanRelation:'giza.live-plan-relation.v1',impactRule:'giza.evidence-impact.v1',tutorial:'giza.tutorial.v1'};d.artifacts=[...paths].sort().map(row);save(p,d);
}
