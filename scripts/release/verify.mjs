...
function tree(dir){return fs.readdirSync(dir,{withFileTypes:true}).flatMap(e=>{if(['node_modules','.git','.cache','dist'].includes(e.name)||e.name.endsWith('.tmp'))return [];const p=(dir==='.'?'':dir+'/')+e.name;return e.isDirectory()?tree(p):[p];});}
