...
async function openAssembly(page:Page){await page.goto('/?spatialDiagnostics=1');await page.getByRole('button',{name:'Sarcophagus',exact:true}).click();const canvas=page.locator('[data-evidence-workbench] canvas');await expect(canvas).toBeVisible();await expect(page.getByLabel('Evidence feature')).toBeVisible();await expect.poll(async()=>JSON.parse(await canvas.getAttribute('data-spatial-diagnostics')??'{}').settled===true).toBe(true);}
